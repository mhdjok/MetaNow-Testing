...
function auth() {
    ...
    if (fullname == null || email == null) {
        ...
        socket.on("connect", () => {
            ...
            socket.emit("user", {fullname : fullname, email : email});
            ...
            users.map(el => {
                if (el.socket_id != MySocketId) {
                    if (el.last_seen == 'Online') {
                        ...
                        $('#users').append("" + "<li id='usersList' city='" + el.socket_id + "' userName='" + el.fullname + "' socketId='" + el.socket_id + "' userId='" + el.id + "' class='clearfix tablinks' ><img src='"+el.profile_image+"' alt='avatar'><div class='about'><div class='name'>" + el.fullname + " </div><div class='status'> <i class='fa fa-circle offline'></i> Last seen at "+ el.last_seen +" </div><div style='display: none' id='num" + el.socket_id + "' class='msgs'></div></div></li>")
                    }
                }
            });
        ...
        $("#fileAttach").change(function (e) {
            ...
            reader.onload = function (evt) {
                ...
                if(isGroup == false) {
                    ...
                    msg.MySocketId = MySocketId;
                    msg.date = date;
                    msg.toUserSocketId = toUserSocketId;
                    msg.group = false;
        ...
        socket.on("message" , function(data) {
            ...
            if(data.group == false) {
                ...
                if (data.toUserSocketId != null && data.toUserSocketId !== toUserSocketId) {
                    document.getElementById('num' + data.fromUser).style.display = 'block';
                    $('#num' + data.fromUser).html('NEW');
                }
                $('#hisT' + data.fromUser).append("<li class='clearfix'><div class=\"message-data \"></div><div class='message my-message'><b>" + data.fullname + "</b><span class='msgTime'>" + moment(data.date).format('h:mm:ss') + "</span><br> " + data.message + " </div></li>");
                $('.chat').animate({scrollTop: 20000000}, "fast");
            }else {
                if (isGroup != true) {
        ...
        socket.on("base64 image", (msg) => {
            if(msg.group == false) {
                ...
                if (msg.id != null && msg.from !== toUserSocketId) {
                    document.getElementById('num' + msg.from).style.display = 'block';
                    $('#num' + msg.from).html('NEW');
                    if(notifiSound) {
                        var audio = new Audio('assets/sounds/notification_sound.mp3');
                        audio.play();
                    }
                }
                $("#hisT" + msg.from).append("<li class='clearfix'><div class=\"message-data \"></div><div style='right: 0' class='message my-message'><b>" + msg.fullname + "</b><span class='msgTime'>" + moment(msg.date).format('h:mm:ss') + "</span><br> <img class='chatImage' src=" + msg.file + " alt=" + msg.message + " /> </div></li>");
                $('.chat').animate({scrollTop: 20000000}, "fast");
            }
            else{
                if (isGroup != true) {
                ...
                $("#hisTgroup").append("<li class='clearfix'><div class=\"message-data \"></div><div style='right: 0' class='message my-message'><b>" + msg.fullname + "</b><span class='msgTime'>" + moment(msg.date).format('h:mm:ss') + "</span><br> <img class='chatImage' src=" + msg.file + " alt=" + msg.message + " /> </div></li>");
                $('.chat').animate({scrollTop: 20000000}, "fast");
            }
        });

        socket.on("attachFile", (msg) => {
            if(msg.group == false) {
                if(notifiSound) {
                    var audio = new Audio('assets/sounds/notification_sound.mp3');
                    audio.play();
                ...
                $("#hisT" + msg.from).append("<li class='clearfix'><div class=\"message-data \"></div><div style='right: 0' class='message my-message'><b>" + msg.fullname + "</b><span class='msgTime'>" + moment(msg.date).format('h:mm:ss') + "</span><br>  <a target=\"_blank\" href='images/"+msg.message+"' style=\"border-radius: 0;font-size: 1.17rem;display: revert;cursor: pointer;width: 100%;color: black;text-decoration: none\" id=\"downloadFile\" download><i class=\"fa fa-download\"></i> <span style='font-size: 15px'> "+msg.message.substring(0,15)+"... </span></a></div></li>");
                $('.chat').animate({scrollTop: 20000000}, "fast");
            }
            else{
                if (isGroup != true) {
                ...
                $("#hisTgroup").append("<li class='clearfix'><div class=\"message-data \"></div><div style='right: 0' class='message my-message'><b>" + msg.fullname + "</b><span class='msgTime'>" + moment(msg.date).format('h:mm:ss') + "</span><br>  <a target=\"_blank\" href='images/"+msg.message+"' style=\"border-radius: 0;font-size: 1.17rem;display: revert;cursor: pointer;width: 100%;color: black;text-decoration: none\" id=\"downloadFile\" download><i class=\"fa fa-download\"></i> <span style='font-size: 15px'> "+msg.message.substring(0,15)+"... </span></a></div></li>");
                $('.chat').animate({scrollTop: 20000000}, "fast");
            }
        });
        ...
        socket.on("voiceRecord", (msg) => {
            if(msg.group == false) {
                ...
                $("#hisT" + msg.from).append("<li class='clearfix'><div class=\"message-data \"></div><div style='right: 0' class='message my-message'><b>" + msg.fullname + "</b><span class='msgTime'>" + moment(dateNow).format('h:mm:ss') + "</span><br>  <audio class='voiceChat' src='./records/"+msg.message+"' controls></audio> </div></li>");
                $('.chat').animate({scrollTop: 20000000}, "fast");
            }
            else{
                if (isGroup != true) {
                ...
                var dateNow = new moment();
                $("#hisTgroup").append("<li class='clearfix'><div class=\"message-data \"></div><div style='right: 0' class='message my-message'><b>" + msg.fullname + "</b><span class='msgTime'>" + moment(dateNow).format('h:mm:ss') + "</span><br><audio class='voiceChat' src='./records/"+msg.message+"' controls></audio> </div></li>");
                $('.chat').animate({scrollTop: 20000000}, "fast");
            }
        });
        ...
        sendVoice.addEventListener("click", function () {
            ...
            if(isGroup == false) {
                ...
                msg.from = MySocketId;
                msg.date = dateNow;
                msg.id = toUserSocketId;
                msg.group = false;
                msg.userId = myUserId;
                msg.toUserId = toUserId;
                socket.emit("voiceRecord", msg);
